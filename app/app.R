  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Multivariate Validity",
        ...
        tabPanel("References",
          ...
          h5("Cortina, J. M., Goldstein, N. B., Payne, S. C., Davison, H. K., & Gilliland, S. W. (2000). The incremental validity of interview scores over and above cognitive ability and conscientiousness scores. ", 
             em("Personnel Psychology, 53, "), "325-351. https://doi.org/10.1111/j.1744-6570.2000.tb00204.x"),
              
          h5("Murphy, K. R. (2019). Understanding how and why adding valid predictors can decrease the validity of selection composites: A generalization of Sackett, Dahlke, Shewach, and Kuncel (2017). ", 
             em("International Journal of Selection and Assessment, 27, "), "249-255. https://doi.org/10.1111/ijsa.12253"),
          
          h5("Voss, N. M., & Lake, C. J. (2020). Communicating validity information to differentially experienced audiences: The effects of numeracy and nontraditional metrics. ", 
...
server <- function(input, output) {
  ...
  output$pred <- renderUI({
    map(numb_pred(), ~ selectizeInput(.x, label = NULL, choices = c(background_predictors, "Other (Backspace to change name)"),
  ...
  output$crit <- renderUI({
    map(numb_crit(), ~ selectizeInput(.x, label = NULL, choices = c(background_criteria, "Other (Backspace to change name)"), 
                                      selected = isolate(input[[.x]]) %||% "Predictor Name", multiple = FALSE, 
  ...
  output$validity <- renderText({c("The validity of your chosen composite is", 
                                   multval_round(), ".")})
  ...
  names_alt <- c("Coefficient of Determination R^2", "Common Language Effect Size CLR", 
  ...
  matrix_alt <- reactive(matrix(c(names_alt, values_alt()), nrow = 4, ncol = 2, 
